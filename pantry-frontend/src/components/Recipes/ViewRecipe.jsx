...
const ViewRecipe = () => {
    ...
    useEffect(() => {
        if (recipe && recipe.id) {
            fetch(`https://localhost:5001/api/recipes/totalPrice/${recipe.id}`)
                .then((response) => {
                    if (response.ok) {
                        return response.json();
                    }
    ...
    useEffect(() => {
        if (recipe && recipe.id) {
            fetch(`https://localhost:5001/api/recipes/pricePerServing/${recipe.id}`, )
                .then((response) => {
                    if (response.ok) {
                        return response.json();
                    }
    ...
    useEffect(() => {
        if (recipe && recipe.id) {
            fetch(`https://localhost:5001/api/measurements?recipeId=${recipe.id}`)//this looks different then normal because its passing data to the api
